/* ===================================================
   엠블럼입출고.js  —  localStorage 버전
   _테스트.html 과 기능/디자인 동일, Supabase 미사용
   =================================================== */
...
══════════════════════════════════════════ */
function 확정id목록불러오기() {
  var raw = localStorage.getItem(확정저장소키);
  if (!raw) { 확정된id목록 = new Set(); return; }
  try { 확정된id목록 = new Set(JSON.parse(raw)); } catch(e) { 확정된id목록 = new Set(); }
}

function 확정id목록저장() {
  localStorage.setItem(확정저장소키, JSON.stringify(Array.from(확정된id목록)));
}
...
function 공정필터목록갱신() {
  ...
  if (현재작업공정) {
    ...
    안내.innerHTML =
      '<b>' + 현재작업공정 + '</b> — 완료: <span class="결과강조">' + 완료 + '건</span>' +
...
function 저장하기() {
  ...
  if (수정중인id !== null) {
    var 이전기록  = 엠블럼_데이터하나가져오기(수정중인id);
    var 이전미완료 = 이전기록 && 이전기록.완료여부 === false;
    if (확정된id목록.has(수정중인id)) { 확정된id목록.delete(수정중인id); 확정id목록저장(); }
    엠블럼_데이터수정(수정중인id, 새항목);
...
function 다음공정자동생성(도착공정, 원출발공정, 출고수량, 품목, lot, 일자) {
  ...
  엠블럼_데이터저장({
    ...
    출고일자: 일자, lot번호: lot, 담당자: '', 담당자코드: '', 완료여부: false
  });
}
...
function 삭제하기(id) {
  확인모달표시('정말 삭제할까요? 되돌릴 수 없습니다.', function() {
    엠블럼_데이터삭제(id);
    if (확정된id목록.has(id)) { 확정된id목록.delete(id); 확정id목록저장(); }
    폼초기화();
    알림표시('삭제되었습니다.', '성공');
    공정필터목록갱신();
    출하현황요약();
...
function lot팝업열기() {
  ...
  조회팝업열기({
    ...
    데이터: Object.values(lot맵).map(function(d) {
...
function lot선택시(lot데이터) {
  ...
  var 관련 = 엠블럼_데이터불러오기().filter(function(h) {
...
function 조회팝업테이블채우기(목록) {
  ...
  목록.forEach(function(항목, 인덱스) {
    ...
    행.addEventListener('click', function() {
      document.querySelectorAll('#조회팝업_테이블바디 tr').forEach(function(r) { r.classList.remove('팝업행키보드선택'); });
      행.classList.add('팝업행키보드선택');
      현재선택행인덱스 = 인덱스;
    });
    행.addEventListener('dblclick', function() {
      현재팝업설정.선택시(항목); 조회팝업닫기();
    });
    바디.appendChild(행);
  });
}
...
function 확정처리() {
  ...
  확인모달표시(선택ids.length + '건을 확정 처리하시겠습니까?', function() {
    확정id목록불러오기();
    var 미확정ids = 선택ids.filter(function(id) { return !확정된id목록.has(id); });
    if (미확정ids.length === 0) { 알림표시('이미 모두 확정된 항목입니다.', '오류'); 공정필터목록갱신(); return; }
    미확정ids.forEach(function(id) { 확정된id목록.add(id); });
    확정id목록저장();
    알림표시(미확정ids.length + '건이 확정 처리되었습니다.', '성공');
    공정필터목록갱신();
  });
}
...
function 폼엔터핸들러(event, 현재id) {
  ...
  if (현재id === '출고일자') {
    ...
    document.getElementById('단가').focus();
    return;
  }
