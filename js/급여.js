  ...
  직원들.forEach(function(직원, idx) {
    ...
    날짜들.forEach(function(날) {
      ...
      if (종류 === '정상출근') {
        ...
        var _지각분 = Number(기록.지각시간) || 0;
        var _외출분 = Number(기록.외출시간) || 0;
        if (_조퇴분 > 0) {
          메인텍스트 = '조퇴';
          실근무 = _조퇴실근무h(_조퇴분, _지각분, _외출분);
          차감h = 8 - 실근무;
        } else {
          차감h = _지각분/60 + _외출분/60;
          실근무 = Math.max(0, 8 - 차감h);
          if (_지각분 > 0 && _외출분 > 0) 메인텍스트 = '지각/외출';
          else if (_지각분 > 0) 메인텍스트 = '지각';
          else if (_외출분 > 0) 메인텍스트 = '외출';
        }
      } else if (종류 === '반차') {
        차감h = 4;
        실근무 = 4;
      ...
      var 시간text = '';
      if (종류 === '주말출근' || 종류 === '공휴일출근') {
        시간text = '8h' + (연장h > 0 ? '+' + 연장h + 'h' : '');
      } else if (실근무 > 0) {
        시간text = (Math.round(실근무 * 10) / 10) + 'h' + (연장h > 0 ? '+' + 연장h + 'h' : '');
      }
    ...
    // 실근무시간 행
    h += '<tr style="height:20px;background:#f0f4ff;">' +
