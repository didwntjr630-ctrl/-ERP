...
/* ===== 등록된 품목 목록 (품명 + 품번) — Supabase 품목마스터 테이블에서 로드 ===== */
var 품목목록 = [];

async function 품목목록로드() {
  var { data, error } = await 수파베이스
    .from('품목마스터')
    .select('*')
    .order('id', { ascending: true });
  if (error) { console.error('품목목록로드 오류:', error); return; }
  품목목록.length = 0;
  (data || []).forEach(function(p) { 품목목록.push(p); });
}
품목목록로드();

수파베이스.channel('품목마스터_실시간_일반')
