...
/* '태산 입고'는 공정검사와 완전히 동일하게 동작 (일단은) */
function 공정검사류(공정) { return 공정 === '공정검사' || 공정 === '태산 입고'; }
...
function 엑셀날짜이번달() {
  var d = new Date();
  var 년 = d.getFullYear();
  var 월 = String(d.getMonth() + 1).padStart(2, '0');
  var 말일 = String(new Date(년, d.getMonth() + 1, 0).getDate()).padStart(2, '0');
  var s = document.getElementById('엑셀시작일');
  var e = document.getElementById('엑셀종료일');
  if (s) s.value = 년 + '-' + 월 + '-01';
  if (e) e.value = 년 + '-' + 월 + '-' + 말일;
}
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  setInterval(async function() {
    if (현재작업공정 !== '출하검사' && !공정검사류(현재작업공정)) return;
...
async function 공정뷰선택(공정) {
  ...
  document.querySelectorAll('.카드 .페이지제목').forEach(function(el) {
    if (el.textContent.trim() === '출하 현황' || el.textContent.trim() === '코팅 출하 현황') {
      el.textContent = (공정검사류(공정)) ? '코팅 출하 현황' : '출하 현황';
    }
  });
...
async function 확정id목록갱신() {
  /* 공정검사·태산입고는 항목.확정됨 컬럼을 직접 보므로 이 함수에서 별도 조회가 필요 없음 */
  if (공정검사류(현재작업공정)) { 확정된id목록 = new Set(); return; }
...
async function 공정필터목록갱신() {
  ...
  if (!현재작업공정) {
    결과 = 전체;
  } else if (현재작업공정 === '수입검사' || 현재작업공정 === '출하검사' || 공정검사류(현재작업공정)) {
...
async function 출하현황요약() {
  var 기준공정 = 공정검사류(현재작업공정) ? 현재작업공정 : '출하검사';
  var 전체 = await 데이터불러오기();
...
async function lot팝업열기() {
  ...
  /* 출하검사: 태산입고에서 확정된 LOT 중, 아직 출하검사에 등록되지 않은 것만 조회해서 수동으로 불러온다 */
  if (현재작업공정 === '출하검사') {
    var 출하lot목록 = new Set(
      전체.filter(function(h) { return h.공정 === '출하검사'; })
          .map(function(h) { return (h['lot번호']||'').trim(); })
    );
    var 확정목록_태산 = 전체
      .filter(function(h) {
        return h.공정 === '태산 입고' && h.확정됨 === true
          && (Number(h.출고수량) || 0) > 0
          && !출하lot목록.has((h['lot번호']||'').trim());
      })
      .map(function(h) {
        return {
          'lot번호':  h['lot번호'] || '-',
    ...
    if (확정목록_태산.length === 0) { 알림표시('태산입고에서 확정된 LOT이 없습니다.', '오류'); return; }

    조회팝업열기({
      제목: '태산입고 확정 LOT 조회 (출하검사로 가져오기)',
      검색힌트: 'LOT 또는 품명 검색...',
      데이터: 확정목록_태산,
      열목록: [
        { 제목: 'LOT 번호',  필드: 'lot번호'  },
...
function 태산입고lot선택시(항목) {
  ...
  폼임시저장();
  작업시작알림();
  var 입고수량el = document.getElementById('입고수량');
...
function 확정처리() {
  ...
  /* 공정검사·태산입고: 다음 공정으로 자동 전달하지 않고, 확정 표시만 해둔다.
     다음 공정(태산입고/출하검사)에서 LOT 조회로 확정된 기록을 수동으로 불러온다. */
  if (공정검사류(현재작업공정)) {
    var 전달공정안내 = 현재작업공정 === '공정검사' ? '태산 입고' : '출하검사';
    확인모달표시(선택ids.length + '건을 확정 처리하시겠습니까?', async function() {
      ...
      알림표시(선택항목.length + '건이 확정 처리되었습니다. ' + 전달공정안내 + '에서 LOT 조회로 불러올 수 있습니다.', '성공');
      await 공정필터목록갱신();
    });
    return;
  }
...
function 폼엔터핸들러(event, 현재id) {
  ...
  // 태산 입고: 품명→일자→LOT까지는 동일, LOT칸에서 엔터 → LOT 조회 팝업(공정검사 확정 데이터 불러오기) 오픈
  // → 팝업에서 선택하면 입고수량으로 포커스 이동 → 입고수량에서 엔터 → 불량내역 팝업
  if (현재작업공정 === '태산 입고' && 현재id === 'lot번호') {
    lot팝업열기();
    return;
  }
  if (현재작업공정 === '태산 입고' && 현재id === '입고수량') {
    불량코드팝업열기();
    return;
  }

  // 공정검사: LOT 다음 입고수량 → A급수량 → 불량내역 팝업 순으로 이동
  if (현재작업공정 === '공정검사' && 현재id === 'lot번호') {
    document.getElementById('입고수량').focus();
    return;
  }
  if (현재작업공정 === '공정검사' && 현재id === '입고수량') {
...
async function 출하검사_엑셀다운로드() {
  ...
  // 검사대장 날짜는 항상 입출고 목록에 보이는 "일자"(출고일자)를 그대로 사용한다.
  // (예전에는 태산입고만 내부 전용 필드 입고일자(공정검사 확정일 스냅샷, 화면 비노출)를 우선 사용했는데,
  //  등록 이후 목록의 일자를 수정해도 이 값은 갱신되지 않아 목록과 검사대장 날짜가 서로 어긋나는 문제가 있었음 — 항상 출고일자 기준으로 통일)
  function 대장일자(항목) { return 항목.출고일자 || (태산입고여부 ? (항목.입고일자 || '') : ''); }
  ...
  if (공정검사여부 || 태산입고여부) {
    ...
    데이터 = 날짜정렬(전체);
    if (데이터.length === 0) {
      알림표시(시작일 + ' ~ ' + 종료일 + ' ' + 업체단축명 + ' 출하 데이터가 없습니다.', '오류');
      if (버튼) { 버튼.disabled = false; 버튼.textContent = '검사대장 출력'; }
      return;
    }
  }
  ...
  try {
    ...
    if (태산입고여부) {
      ...
      파일명 = 업체단축명 + '출하검사대장_' + 시작일.replace(/-/g,'') + '~' + 종료일.replace(/-/g,'') + '.xlsx';
      ...
      ws.getCell('A1').value = 업체타이틀 + ' 출 하 검 사 대 장 ( ' + 시작일 + ' ~ ' + 종료일 + ' )';
      시트채우기(ws, 데이터, 25);
