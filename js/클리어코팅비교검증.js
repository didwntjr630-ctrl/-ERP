...
async function _클리어코팅_매칭(로트측정값맵) {
  ...
  Object.keys(로트측정값맵).forEach(function(key) {
    ...
    var 레코드 = 코팅입고맵[key];
    if (!레코드) {
      미매칭목록.push({ lot: key, 사유: '전산 코팅 입고(태산 입고) 데이터에 없는 LOT' });
      return;
    }
...
/* ─────────── 메인: 업로드 파일 → 매칭 → 월별 워크북 생성 → 다운로드 ─────────── */
async function 클리어코팅_생성및다운로드(파일) {
  ...
  if (!매칭결과.매칭목록.length) {
    throw new Error('전산 코팅 입고 데이터와 매칭된 LOT이 하나도 없습니다.');
  }

  var 월별 = {};
  매칭결과.매칭목록.forEach(function(rec) {
