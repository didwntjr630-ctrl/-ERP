/* ===================================================
   ...
   전부 전산 데이터 기준으로 채운다. 기입되는 측정값은 원본에 0.3을 보정해서 넣으며,
   합격/불합격 판정은 기존 차종별 규격(CN7 PE 15~21, 그 외 4~8)을 그대로 쓰되,
   비고 칸에 색상별 별도 검증(S/V는 7.1~7.9 범위, B/K는 경계값 15·21 여부)을 추가로 표시한다.
   =================================================== */
...
function _클리어코팅_월시트채우기(workbook, 연월, 레코드목록) {
  ...
  정렬.forEach(function(rec, idx) {
    ...
    /* 첨부파일 원측정값에 0.3 보정을 더해서 기입 (소수 1자리로 반올림 — 부동소수점 오차 방지) */
    ['H', 'I', 'J', 'K', 'L'].forEach(function(col, i) {
      ws.getCell(col + 행).value = Math.round((rec.측정값[i] + 0.3) * 10) / 10;
    });
    ...
    /* 비고: 기존 합격/불합격 판정은 그대로 두고, 보정값(0.3 더한 값) 자체 검증을 별도로 덧붙임 —
       S/V는 7.1~7.9 범위, B/K는 경계값(15, 21)에 정확히 걸리는지만 확인 */
    ws.getCell('Q' + 행).value = { formula:
      'IF(P' + 행 + '="불합격", "규격 이탈 확인", ' +
        'IF(G' + 행 + '="S/V", ' +
          'IF(OR(MIN(H' + 행 + ':L' + 행 + ')<7.1, MAX(H' + 행 + ':L' + 행 + ')>7.9), "보정값 범위(7.1~7.9) 이탈 확인", "-"), ' +
