...
/* 거래처(모델)별 상세표(99행 이하) — 그룹 번호를 특정 모델에 고정하지 않고,
   그날 출하검사 데이터가 있는 모델만 이 순서대로 1번 그룹부터 빈칸 없이 채운다 (MX5 제외) */
var 품질현황_상세표_모델순서 = [
  { 품명목록: ['기아 61mm'],                                     표시명: '61MM'     },
  { 품명목록: ['기아 71mm'],                                     표시명: '71MM'     },
  { 품명목록: ['H-EMBLEM SILVER(CN7 PE)', 'H-EMBLEM BLACK(CN7 PE)'], 표시명: 'H EMBLEM' },
  { 품명목록: ['BEZEL(GN7)'],                                    표시명: 'GN7 BEZEL' },
  { 품명목록: ['기아 45mm'],                                      표시명: '45MM'     },
  { 품명목록: ['GN7 F/L'],                                       표시명: 'GN7 F/L'  }
];
var 품질현황_상세표_전체그룹헤더행 = [99, 120, 141, 162, 183, 204, 225, 246]; /* 초기화용 — MX5 몫(246)까지 포함 전체 8개 슬롯 */
...
function _품질현황_상세표초기화(xml) {
  품질현황_상세표_전체그룹헤더행.forEach(function(헤더행) {
    var 첫데이터행 = 헤더행 + 3;
    xml = _품질현황_셀지우기(xml, 'D' + 첫데이터행); // 그룹 모델명 표시(병합 셀)도 초기화
    for (var t = 0; t < 5; t++) {
...
function _품질현황_상세표채우기(xml, 하루기록_출하검사, 공정검사_lot맵) {
  ...
  // 품명별로 묶되, 그날 실제 데이터가 있는 모델만 정해진 순서대로 골라 1번 그룹부터 빈칸 없이 배정한다
  var 품명별 = {};
  하루기록_출하검사.forEach(function(항목) {
    if (!품명별[항목.품명]) 품명별[항목.품명] = [];
    품명별[항목.품명].push(항목);
  });

  var 채울그룹목록 = [];
  품질현황_상세표_모델순서.forEach(function(모델) {
    var 목록 = [];
    모델.품명목록.forEach(function(품명) {
      if (품명별[품명]) 목록 = 목록.concat(품명별[품명]);
    });
    if (목록.length) 채울그룹목록.push({ 표시명: 모델.표시명, 목록: 목록 });
  });

  채울그룹목록.slice(0, 품질현황_상세표_전체그룹헤더행.length).forEach(function(그룹, 그룹idx) {
    var 헤더행 = 품질현황_상세표_전체그룹헤더행[그룹idx];
    var 첫데이터행 = 헤더행 + 3;
    xml = _품질현황_셀쓰기(xml, 'D' + 첫데이터행, 그룹.표시명, true);

    var 정렬된 = 그룹.목록.slice().sort(function(a, b) { return (a.id || 0) - (b.id || 0); });
