...
/* 동일 모델 슬롯 구조가 40행 아래(45~84행)에 그대로 반복 — 출하검사 데이터 → 태산 구역 */
var 품질현황_모델슬롯맵_태산 = {
  '기아 61mm':               { 시작행: 45 },
  '기아 71mm':               { 시작행: 50 },
  ...
  'MX5 BEZEL SILVER':        { 시작행: 80 },
  'MX5 BEZEL BLACK':         { 시작행: 80 }
};
...
function _품질현황_일자시트작성(xml, 연, 월, 일, 하루기록_삼양, 하루기록_태산) {
...
async function 품질현황_생성(연, 월) {
  ...
  // 출하검사는 LOT 조회로 태산입고에서 넘어와 바로 등록되어 자체 불량내역이 없으므로,
  // 같은 LOT의 태산입고 기록에서 불량내역을 가져와 채움 (일자는 출하검사 자체 출고일자 그대로 사용)
  var 태산입고_불량맵 = {};
  전체.forEach(function(h) {
    if (h.공정 === '태산 입고' && h['lot번호']) {
      태산입고_불량맵[h['lot번호']] = h.불량내역 || [];
    }
  });
  var 대상_출하검사 = 전체.filter(function(h) {
    ...
    return Object.assign({}, h, { 불량내역: 태산입고_불량맵[h['lot번호']] || h.불량내역 || [] });
  });
  ...
  for (var 일 = 1; 일 <= 말일; 일++) {
    ...
    xml = _품질현황_일자시트작성(xml, 연, 월, 일, 일자별[일], 일자별_출하검사[일]);
    zip.file(시트파일, xml);
  }
