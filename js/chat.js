...
var _숨긴DM = {};   // { roomId: true } — 나가기한 DM 방 숨김 목록
  ...
  if (window._접속자채널) {
    ...
    Object.keys(state).forEach(function(key) {
      ...
      if (항목들.length > 0 && 항목들[0].사원명 && 항목들[0].사원명 !== 세션.사원명) {
        var rId = _DM방id(세션.사원명, 항목들[0].사원명);
        if (!_숨긴DM[rId]) {
  ...
  _채팅채널 = 수파베이스
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: '채팅메시지' }, function(payload) {
      ...
      if (_채팅열림 && msg.방id === _현재방) {
        ...
        // 숨겨진 DM 방에 새 메시지가 오면 목록 복원
        if (_숨긴DM[msg.방id] && _내방인가(msg.방id, 세션.사원명) && msg.발신자명 !== 세션.사원명) {
          delete _숨긴DM[msg.방id];
          DM목록갱신();
        }
        안읽음수갱신();
